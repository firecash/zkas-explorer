...
export default function NodeGlobe({ nodes, labels, activeId, onHover, onSelect, txs, blocks, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    const buildLabels = () => {
      ...
      labelEls = labelsRef.current.map((c) => {
        ...
        // textContent, not innerHTML: country names/counts come off the API.
        ...
        if (labelsCompact) {
          mk(flagOf(c.code));
          mk(String(c.count), "color:#17d6be;margin-left:4px;font-size:12px;font-weight:600");
        } else {
          mk(flagOf(c.code) + " ", "opacity:.95");
          mk(c.name);
          mk(String(c.count), "color:#17d6be;margin-left:6px");
        }
  ...
  return <div ref={mountRef} className="relative h-full w-full cursor-grab touch-pan-y" />;
}
